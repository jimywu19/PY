...
def sortLineText(pdfLayOut):
    '''
    把PDF内文字字段按从左到右顺序解析取值
    https://github.com/euske/pdfminer/issues/290
    '''
    ...
    # for elt in selts:
    #    print(f"({elt['x0']:0.0f},{elt['y0']:0.0f},{elt['x1']:0.0f},{elt['y1']:0.0f}){elt['txt']}")

    # if not callback(p,selts,"something is wrong while sortlinetxt"):
    #     return
    return selts
def parse(file,savefile):
    ...
    if not doc.is_extractable:
        ...
        for page in doc.get_pages():
            ...
            sortedElements = sortLineText(layout)               
            #p += 1
            elt_len = len(sortedElements)
            
            for x in range(elt_len):
                if sortedElements[x]['txt'] == '联系人：' :
                    result['联系人'] = sortedElements[x+1]['txt']
                elif sortedElements[x]['txt'] == '业务开通日期：' :
                    result['开通日期'] = sortedElements[x+1]['txt']
                #elif sortedElements[x]['txt'] == ''
                else:
                    # ipAddress = getIp(sortedElements[x]['txt'].replace(" ",""))
                    ipAddress = getIp(sortedElements[x]['txt'])
                    if ipAddress:
