...
getStatus_double()
{
    ...
    case "$LOCAL_ROLE" in
        ...
        "$gs_r_normal"|"$gs_r_primary"|"$gs_r_standby"|"$gs_r_unknown"|*)
...
doDeactivate()
{
    ...
    # 主降备，在操作前，需要看对端的状态
    if [ "" == "$PEER_ROLE" ] || [ "$gs_r_primary" == "$PEER_ROLE" ] || [ "$gs_r_cstandby" == "$PEER_ROLE" ]; then
        restartToCStandby1; retVal=$?
        return $retVal
    fi
