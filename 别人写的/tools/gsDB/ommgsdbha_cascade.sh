...
LOG_FILE="/var/log/goku/ha/shelllog/dbmonitor.log"
...
if ! . "${gCurPath}/dbcommand_cascade.sh" ; then
    LOG_ERROR "load ${gCurPath}/dbcommand_cascade.sh failure!"
    exit 2
fi

DB_PROG_DIR=/opt/goku/dbprogram
...
doRepair_double()
{
    ...
    # 不为备状态，或是级联备，不处理
    if [ "$LOCAL_ROLE" != "$gs_r_standby" -a "$LOCAL_ROLE" != "$gs_r_cstandby" -a "$LOCAL_ROLE" != "$gs_r_cstandby1" ]; then
...
StaToPri()
{
    ...
    if [ $retVal -ne $r_success ] || ! checkLocalRole "$gs_r_cstandby"; then
...
StaForceToPri()
{
    ...
    local dbPwd=$(sudo /usr/local/bin/pwswitch -D key.so -fp "$fpr1nt")
    # 备升主，强制切换 failover
    $OMM_GS_CTL -P "$dbPwd" failover -M "$gs_c_cstandby"
    unset dbPwd
    [ $retVal -eq 0 ] && relaodFloatIp; retVal=$?
...
notifyToPrimary()
{
    ...
    local dbPwd=$(sudo /usr/local/bin/pwswitch -D key.so -fp "$fpr1nt")
    # 通知数据库，将状态切换为主用
    $OMM_GS_CTL -P "$dbPwd" notify -w -M $gs_c_cstandby; retVal=$?
    unset dbPwd

    # 通知失败
    if [ $retVal -ne $r_success ]; then
        LOG_ERROR "[notifyToPrimary] call [$OMM_GS_CTL notify -M $gs_c_cstandby] failure[$retVal]."
        return $r_failure
    fi
...
notifyToStandby()
{
    ...
    local dbPwd=$(sudo /usr/local/bin/pwswitch -D key.so -fp "$fpr1nt")
    # 通知数据库，将状态切换为备
    $OMM_GS_CTL -P "$dbPwd" notify -w -M $gs_c_cstandby1; retVal=$?
    unset dbPwd

    # 通知失败
    if [ $retVal -ne $r_success ]; then
        LOG_ERROR "[notifyToStandby] call [$OMM_GS_CTL notify -M $gs_c_cstandby1] failure[$retVal]."
        return $r_failure
    fi
