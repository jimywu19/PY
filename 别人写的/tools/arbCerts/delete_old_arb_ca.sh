...
echo "`date` start to delete old arb cert">>$logfile
...
#1.生成新的keystore
awk '{if( $0 ~ /-*BEGIN CERTIFICATE-*/){i=i+1;output="sub_ca"i".crt";print $0>>output}else{print $0>>output}}' $cert_path
i=1
...
echo "`date` backup original keystore">>$logfile
...
if [ $test -eq 1 ];then
  echo "`date` test delete old arb successfully">>$logfile
